...
class SequelizeMocking {
    ...
    static copyModel(sequelizeInstance, model) {
        let newModel = new Sequelize.Model(
            ...
            _.merge({}, model.options, { sequelize: sequelizeInstance })
        );

        newModel = newModel.init(sequelizeInstance.modelManager);
        sequelizeInstance.modelManager.addDAO(newModel);
        return newModel;
    }
    ...
    static create(originalSequelize, options) {
        ...
        let mockedSequelize = new Sequelize(
            FAKE_DATABASE_NAME,
            null,
            null,
            SequelizeMocking.adaptSequelizeOptions(originalSequelize, options)
        );
        ...
        return mockedSequelize.sync().then(function() {
            logging &&
                console.log('SequelizeMocking - Database construction done');
            return mockedSequelize;
        });
    }
     ...
     * @returns {Promise.<Sequelize>}
     */
    static createAndLoadFixtureFile(
        originalSequelize,
        fixtureFilePath,
        options
    ) {
        ...
        return SequelizeMocking.create(originalSequelize, options).then(
            function(mockedSequelize) {
                return SequelizeMocking.loadFixtureFile(
                    mockedSequelize,
                    fixtureFilePath,
                    options
                ).then(function() {
                    logging &&
                        console.log('SequelizeMocking - Mocked data injected');
                    return mockedSequelize;
                });
            }
        );
    ...
    static hookNewModel(originalSequelize, mockedSequelize, options) {
        ...
        originalSequelize.addHook(AFTER_DEFINE_EVENT, function(newModel) {
            SequelizeMocking.modifyModelReference(
                mockedSequelize,
                ...
                .then(function() {
                    logging &&
                        console.log(
                            `Model ${newModel.name} was declared into the database`
                        );
                })
                .catch(function(err) {
                    logging &&
                        console.error(
                            `An error occured when initializing the model ${newModel.name}`
                        );
    ...
    static loadFixtureFile(sequelize, fixtureFilePath, options) {
        ...
        return sequelizeFixtures
            .loadFile(fixtureFilePath, SequelizeMocking.mapModels(sequelize), {
    ...
    static modifyConnection(originalSequelize, newSequelizeToUse) {
        originalSequelize.__connectionManager =
            originalSequelize.connectionManager;
        originalSequelize.connectionManager =
            newSequelizeToUse.connectionManager;
    ...
    static restore(mockedSequelize, options) {
        ...
        if (mockedSequelize.__originalSequelize) {
            SequelizeMocking.modifyModelReferences(
                mockedSequelize,
                mockedSequelize.__originalSequelize
            );
        }

        if (mockedSequelize.__dialect && mockedSequelize.__connectionManager) {
            SequelizeMocking.modifyConnection(
                mockedSequelize,
                mockedSequelize.__originalSequelize
            );
        }
